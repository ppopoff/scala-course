...
обычно называется “Cons”. Более того, `::` — один из редких операторов Scala,
который обладает правой ассоциативностью, поэтому, увидев Nil справа,
компилятор догадывается что мы конструируем список и вызывает метод `::`
получив аргумент в который, компилятор в состоянии определить тип списка.
Более подробно об этом написано в книге Одерского ”Programming in scala”.
К тому же, использование Nil более [идиоматично][5], чем использование
`List.empty` или `List()`
...
Литература
==========
Более подробно о списках и других типах коллекций можно прочитать на сайте
[Twitter Scala School][tschool-col]. У Alvin Alexander существует ряд неплохих
руководств [здесь][aal1] [здесь][aal2] и [здесь][aal3]. Официальная
[документация][list-doc] всегда будет кстати. Как идиоматично обозначать
