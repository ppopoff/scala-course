...
## Введение
Как вы могли заметить, существует два способа
...
Использование типа зависит от ситуации. Однако что в первом, что во
...
Выше продемонстрирована функция, которая не принимает и не возвращает аргументы.
Если у вас используются Generic-параметры, вполне допустимо
использовать для `Function1` функциональный стиль прямо на месте:

    def map[B](f: A => B) = ...

Указание типа функций для функциональных языков имеет смысл, т. к. в этих
языках есть возможность принимать и передавать функции в качестве
аргументов. Но обо всем по порядку.

## О хороших функциях и композиции
...
рассказывать о функциях с арностью 1 ввиду наличия у них особых свойств.

Композиция функций ([function composition][fun-comp]) — простейший и основной
способ комбинирования функций, основанный на размещении функций таким образом,
что выход одной функции — вход для другой. Давайте рассмотрим две простые
функции:

    def inc (x: Int) = x + 1
    def dup (x: Int) = x * 2

Наша задача — собрать функцию, которая принимает целое
...
многих функциональных языках, например, в Haskell, есть поддержка особого
...
Scala — не исключение, в ней тоже есть аналогичная операция, которая,
...
Скорее всего, вы уже заметили что конструкции следует читать
...
что у вас наличествует представление о них. Про Underscore synta (синтаксис с
...
небольшое приложение, которое чертит на экране график заданной функции. В
первом приближении это может выглядеть так:
...
Как видите, функции math.cos и math.sin замечательно себя
зарекомендовали. Написанный код будет работать для любой функции типа
`Double => Double`.

Когда нужно возвращать другую функцию?
Наиболее распространенный случай — каррирование, про него расскажем ниже.
Достаточно понятный пример вы можете найти [здесь][high-order-2]. Также про
функции высшего порядка можно прочесть ы [здесь][high-order-1].


## Каррирование (Currying)
[Техника][curry-0], которая позволяет рассматривать функцию с несколькими
аргументами, в цепочке функций,принимающих один аргумент. В некоторых
функциональных языках, таких как Haskell, все функции каррируются по умолчанию
(и ничего страшного не происходит). Каррирование — достаточно сложная тема,
...
> с несколькими аргументами на цепочку функций с одним аргументом, которые
...
Перед тем как вы глубоко погрузитесь в каррирование, мы бы хотели, чтобы вы
...
Литература
==========
 - Узнать больше про композицию функций можно на сайте
